...
################################################################################
# YOLO Label Verifier - Simple Multi-GPU Script (SAM3 Style)
# Usage: ./run_multigpu_simple.sh [num_gpus]
################################################################################

# GPU 개수 (인자로 받거나 자동 감지)
...
temp_input = temp_dir / 'JPEGImages'
temp_input.mkdir(parents=True, exist_ok=True)

# 이미지 복사 (symlink 대신 항상 복사)
for img_path in image_paths:
    if img_path.exists():
        dst = temp_input / img_path.name
        if not dst.exists():
            shutil.copy2(img_path, dst)

# Config 수정
config['paths']['input_images'] = str(temp_input)
...
# Config path 덮어쓰기
verifier.Path.__class__.parent = property(lambda self: temp_dir)
original_config_path = verifier.Path(__file__).parent / 'config.yaml'
...
# 검증 실행
input_images = Path(config_loaded['paths']['input_images'])
...
image_files = sorted(list(input_images.glob('*.jpg')) + 
                    ...
                    list(input_images.glob('*.JPG')) + 
                    list(input_images.glob('*.JPEG')) + 
                    list(input_images.glob('*.PNG')))

print(f"[GPU {gpu_id}] Found {len(image_files)} images to process")
...
for idx, image_path in enumerate(image_files):
    ...
    try:
        ...
        if (idx + 1) % 10 == 0:
            print(f"[GPU {gpu_id}] Progress: {idx + 1}/{len(image_files)}")
...
for i in $(seq 0 $((NUM_GPUS - 1))); do
    ...
    if [ -f "$SPLIT_FILE" ]; then
        echo "[GPU $i] Starting..."
        CUDA_VISIBLE_DEVICES=$i python3 run_gpu_worker.py $i $SPLIT_FILE > gpu${i}.log 2>&1 &
    fi
done
